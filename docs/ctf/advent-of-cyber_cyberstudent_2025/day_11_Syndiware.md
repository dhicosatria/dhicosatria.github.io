...
[**syndicate_locker.zip**](https://files.vipin.xyz/api/public/dl/mZx3JOra/syndicate_locker.zip)

## Understand How Ransomware Works

Open `FreeRobux.py`. The gist of it is as follows:
...
- **Keys are stored in memory (VirtualAlloc)**
- **Static markers `AA BB CC DD`** are present
- **Memory dump: `ransomware.DMP`** are present

## Data Structure in Memory

Each file is stored in memory like this:
...
1. Find marker `AA BB CC DD`
2. Move back 32 bytes → **KEY**
3. Move back 60 bytes → **FILENAME**

## Extract Key from Memory Dump

Run this script in the same folder as `ransomware.DMP`:
...
![image.png](media/day11_2.png)

## Check the recovered file

```bash
file recovered_Elf\ 41\'s\ Diary.pdf
file recovered_ourking.png

```

![image.png](media/day11_3.png)

decode PDF

```jsx
with open("decoded.txt", "w") as out:
